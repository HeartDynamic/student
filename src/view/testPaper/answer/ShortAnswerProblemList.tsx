...
const ShortAnswerProblemList: FC = props => {
    ...
    //选项
    const handleChangeStudentAnswer = (value: Value) => {
        answerStore.testProblemDetailData.studentAnswer = value
    }

    //保存答案
    const handleClickSave = () => {
        answerStore.setAnswer = {
            studentTestId: answerStore.testProblemData.id,
            testVolumesProblemId: answerStore.testProblemDetailData.id,
            studentAnswer: JSON.stringify(answerStore.testProblemDetailData.studentAnswer),
            type: answerStore.currentProblemDetailData.type,
        }
        ;(answerStore.testProblemData as any)[answerStore.currentProblemDetailData.type].filter((item: any) => {
            if (item.number === answerStore.currentProblemDetailData.number) {
                ...
                <ScrollbarWrap>
                    ...
                    <Package1>
                        ...
                        <Wrap>
                            ...
                            <Answer>
                                <Editor
                                    value={answerStore.testProblemDetailData.studentAnswer}
                                    onChange={handleChangeStudentAnswer}
                                ></Editor>
                            </Answer>
                        </Wrap>
                    </Package1>
